...
for i,v in enumerate(T_in):
    ...
    h_in = rp.tp(T_in[i],p_in[i]*1000,"Isobutane*Propane*Propylen",composition=[xa[i],xb[i],1-xa[i]-xb[i]],option=1,units=_units,props=_props)[2]

    with open('h_in.txt','a') as f:
             f.write(str(h_in)+'\n')
